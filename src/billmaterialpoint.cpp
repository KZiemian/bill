...
bill::BillMaterialPoint::BillMaterialPoint(bill::BillIntegrator algorithm,
  ...
  past    = std::pair<bill::vector, bill::vector>(position
						 - velocity * step, {0,0,0});
...
void bill::BillMaterialPoint::x(bill::vector new_x) {
  ...
  past = std::pair<bill::vector, bill::vector>(std::get<0>(present)
...
std::shared_ptr<bill::BillMaterialPoint>
bill::BillSetOfPoints::operator[](size_t n) {
  if(n >= points.size()) {
    std::cerr<<("bill::BillSetOfPoints::operator[]:"
