...
bill::vector bill::BillRigidBody::Force() {
  return bill::vector({0.0, 0.0, 0.0});
}
...
void bill::BillRigidBody::CalculateMove(bill::vector& force,
					bill::vector& torque) {
...
bill::BillRigidBody::BillRigidBody(bill::BillRBIntegrator algorithm,
				   bill::vector position,
				   bill::vector velocity,
				   quaternion rotation, vector angular,
				   double mass, bill::vector color,
				   double step) {
  this->algorithm = algorithm;
  this->step = step;

  past = std::tuple<bill::vector, bill::vector, bill::quaternion,
		    bill::vector>(position - velocity * step, {0, 0, 0},
				  rotation, angular);
  present = std::tuple<bill::vector, bill::vector, bill::quaternion,
		       bill::vector>(position, velocity, rotation, angular);
  future  = std::tuple<bill::vector, bill::vector, bill::quaternion,
		       bill::vector>({0, 0, 0}, {0, 0, 0}, rotation,
				     angular);
...
void bill::BillRigidBody::x(bill::vector new_x) {
  ...
  past = std::tuple<bill::vector, bill::vector, bill::quaternion,
		    bill::vector>(std::get<0>(present)
				  - std::get<1>(present) * step,
				  std::get<1>(present),
				  std::get<2>(present),
				  std::get<3>(present));
}
...
std::shared_ptr<bill::BillRigidBody> bill::BillSetOfRigidBodies::operator[](size_t n) {
  if (n >= bodies.size()) {
    std::cerr<<("bill::BillSetOfbodies::operator[]: "
