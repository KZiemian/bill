...
box::box(bill::BillRBIntegrator algorithm, double _a, double _b, double _c,
	 bill::vector position, bill::vector velocity,
	 bill::quaternion rotation, bill::vector angular)
  :bill::BillRigidBody(algorithm, position, velocity, rotation, angular) {
  a = _a;
  b = _b;
  c = _c;
...
void box::rotatedcoordinates(bill::vector& i, bill::vector& j,
			     bill::vector& k) {
  i = bill::vector({1.0, 0.0, 0.0});
  j = bill::vector({0.0, 1.0, 0.0});
  k = bill::vector({0.0, 0.0, 1.0});
...
void box::Draw(){
  ...
  bill::GLaux::drawBox(points, colors, 1.0);
}
...
bill::vector box::get_versor(const unsigned int i) {
  bill::vector result({0.0, 0.0, 0.0});
  result[i] = 1.;
  rotatevector(result);
  return result;
