...
bill::BillRBIntegrator HollyWood = [](std::tuple<bill::vector,
				      bill::vector, bill::quaternion,
				      bill::vector> PhasePoint0,
				      std::tuple<bill::vector, bill::vector,
				      bill::quaternion, bill::vector>
				      PhasePointM, bill::vector Force,
  ...
  bill::quaternion qw (0.0, w);

  q = q + step * 0.5 * (qw * q);
  x += step * v;

  return std::tuple<bill::vector, bill::vector, bill::quaternion,
  bill::vector>(x, v, q, w);
};
  ...
  virtual void Act(){
    for(unsigned int i = 0; i < bodies.size(); ++i)
      for(unsigned int j = 0; j < i; ++j)
        if (bill::vector::norm(bodies[i]->future_position()
			       - bodies[j]->future_position())<
	    bodies[i]->BS() + bodies[j]->BS()) {
          bodies[i]->disable();
          bodies[j]->disable();
        }
...
int main(int argc, char **argv) {
  ...
  SetOfBodies.AddBody(new box(HollyWood, 0.1, 0.2, 0.3,
			      bill::vector({0.0, 0.0, 0.3})));
  SetOfBodies.AddBody(new box(HollyWood, 0.1, 0.1, 0.1,
			      bill::vector({0.0, 0.0, -0.3}),
			      bill::vector({0.0, 0.0, 0.02}),
			      bill::quaternion({0.0, 0.0, 1.0, 0.0}),
			      bill::vector({0.2, 0.0, 0.0})));
