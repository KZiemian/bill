...
namespace bill{
  ...
  using BillRBIntegrator = std::function<std::tuple<vector, vector,
    quaternion, vector>(std::tuple<vector, vector, quaternion, vector>,
			std::tuple<vector, vector, quaternion, vector>,
			vector, double)>;
  ...
  protected:
    ...
    std::tuple<vector, vector, quaternion, vector> past; // 0-> position
    /* 1-> velocity 2-> rotation 3-> angular velocity */
  ...
  public:
    BillRigidBody(BillRBIntegrator algorithm, vector position
		  = vector({0.0, 0.0, 0.0}),
		  vector velocity = vector({0.0, 0.0, 0. }),
		  quaternion rotation = quaternion({0.0, 0.0, 1.0, 0.0}),
		  vector angular = vector({0.1, 0.1, 0.1}),
		  double mass = 1.0,
		  vector color = vector({1.0, 0.0, 0.0}), double step = 0.2);
