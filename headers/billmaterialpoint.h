...
namespace bill {
  ...
  using BillIntegrator =
    std::function<std::pair<vector, vector>(std::pair<vector, vector>,
					    std::pair<vector, vector>,
					    vector, double)>;
  
  class BillMaterialPoint {
  public:
    BillMaterialPoint(BillIntegrator algorithm, vector position
		      = vector({0., 0., 0.}),
		      vector velocity = vector({0., 0., 0.}),
		      double mass = 1.0,
		      vector color = vector({1.0, 0.0, 0.0}),
		      double step = 0.2);
