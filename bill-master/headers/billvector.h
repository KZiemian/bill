...
namespace bill {
  ...
  public:
    ...
    vectorNd<T, N>(std::initializer_list<T> list) {
      if (list.size() != N) {
	std::cerr << ("bill::vector<T, N>: initialization list is not"
		      " the size of vector! Additional numbers"
		      " are removed!\n");
      }
    ...
    vectorNd<T, N>(std::vector<T> vec) {
      if (vec.size() != N)
	std::cerr << ("bill::vector<T,N>: initialization vector is not"
		      " the size of vector! Additional numbers are"
		      " removed!\n");
    ...
    T operator[](size_t n) const {
      if ((n >= N) || (n < 0)) {
	std::cerr << ("bill::vector<T,N>::operator[]: there is "
		      "no field to access!\n");
	...
	return vectorNd<T, N>();
      ...
      if (N != 3) {
	std::cerr << ("bill::vector<T,N>::operator^: vector product "
		      "exist only in 3 dimensions!\n");
  ...
  quaternion(double valueV, double valueS): s(valueS), v(valueV) {
	...
	v = vector(tmp);
      ...
      if (list.size() > 4) {
	std::cerr << ("bill::quaternion: initialization list is not "
		      "the size of vector! Additional numbers "
		      "are removed!\n");
	std::vector<double> tmp;
  ...
  quaternion(double valueS, std::initializer_list<double> list): s(valueS) {
	...
	v = vector(list);
      ...
      if (list.size() > 3) {
	std::cerr<<("bill::quaternion: initialization vec is not "
		    "the size of vector! Additional numbers are removed!\n");
	v = vector(list);
      }
    }
  ...
  quaternion(vector versor, double theta)
    ...
    double operator[](int n) const {
      if ((n >= 3) || (n < -1)) {
	std::cerr << ("bill::quaternion::operator[]: there is no field "
		      "to access!\n");
    ...
    double& operator[](int n) {
      if ((n >= 3) || (n < -1)) {
	std::cerr<<("bill::quaternion::operator[]: "
		    "there is no field to access!\n");
    ...
    friend quaternion operator*(const double & scalar,
    ...
    static void rotate(vector& vec, const vector& axis,
		       const double& angle) {
